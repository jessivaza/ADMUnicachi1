...
public class ClienteDAO implements CRUDCliente {
   ...
   public int saveCliente(Cliente cliente) {
        ...
        try {
            cn = new MySqlConectar().getConnection();
            String sql = "INSERT INTO cliente (nombre, apellido, direccion, dni, email, contrasena, telefono) VALUES (?,?,?,?,?,?,?)"; // Se agrega dni
            pstm = cn.prepareStatement(sql);
    ...
    public int updateCliente(Cliente cliente) {
        ...
        try {
            cn = new MySqlConectar().getConnection();
            String sql = "UPDATE cliente SET nombre=?, apellido=?, direccion=?, dni=?, email=?, contrasena=?, telefono=? WHERE idCliente=?";
            pstm = cn.prepareStatement(sql);
            pstm.setString(1, cliente.getNombre());
            pstm.setString(2, cliente.getApellido());
            pstm.setString(3, cliente.getDireccion());
    ...
    @Override
    public int deleteClienteById(int idCliente) {
        int salida = -1;
        Connection cn = null;
        PreparedStatement pstm = null;
        try {
